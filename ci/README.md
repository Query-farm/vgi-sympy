# CI: the vgi-sympy worker integration suite

[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs the unit tests
and this repo's sqllogictest suite (`test/sql/*.test`) against the vgi-sympy
...
1. **Install the worker** — `uv sync --frozen` into a venv. `sympy_worker.py`
   is a self-contained PEP 723 stdio worker the extension can spawn via
   `uv run sympy_worker.py`.
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, points `VGI_SYMPY_WORKER` at `uv run sympy_worker.py`, warms the
...
VGI_SYMPY_WORKER="uv run --python 3.13 sympy_worker.py" \
  ci/run-integration.sh
```

Or use the Makefile target `make test-sql`, which installs `haybarn-unittest`
as a uv tool and points the worker at `uv run --python 3.13 sympy_worker.py`.
